...
class code_coverage:
	...
	#print "Calling addr_to_module"
	# For macs we use dlls....
        mod32 = pydbg.addr_to_dll(pydbg.context.Eip)

        if mod32:
            module = mod32.name
            base   = mod32.base
	    #print "Got module %s with base %x" % (module, base)


	#print "Adding CC hit %x with base %x\n" % (pydbg.context.Eip, base)
    ...
    def export_mysql (self, target_id, tag_id):
        ...
        for hits in self.hits.values():
            for ccs in hits:
                ...
                sql += "     module       = '%s'," % string.lower(ccs.module)
                sql += "     base         = '%d' " % ccs.base
