...
class pydbg_core(object):
    ...
    system_modules    = []	   # list of modules
	...
	#print "_________ attaching in pydbg ________ "
        # obtain necessary debug privileges.
        self.get_debug_privileges()
	#print "_________ got debug privs ___________ "
        self.pid       = pid
        self.h_process = self.open_process(pid)

	#print "_________ opened process ____________ "
        self.debug_active_process(pid)


	# for macs, we iterate all the loaded libraries and store them
	region_line_re = re.compile('.*([0-9a-f]{8})-([0-9a-f]{8})\s+\[.+\]\s+[r-][w-][x]/[r-][w-][x-]\s+[^\s]+\s+([^\s]+)')
	base = {}
	end = {}

    	(dummy,fp) = os.popen4("vmmap -w %d"%(pid,))
    	for x in fp:       
        	y = region_line_re.search(x)
        	if y:
        		z = y.groups()
			# pick smallest base
			if (base.has_key(os.path.basename(z[2]))):
				if(int(z[0],16) < base[os.path.basename(z[2])]):
					base[os.path.basename(z[2])] = int(z[0],16)
			else:
				base[os.path.basename(z[2])] = int(z[0],16)
                        if (end.has_key(os.path.basename(z[2]))):
                                if(int(z[1],16) > end[os.path.basename(z[2])]):
                                        end[os.path.basename(z[2])] = int(z[1],16) 
                        else:   
                                end[os.path.basename(z[2])] = int(z[1],16)

	for k in base.keys():
		dll = system_dll(k, base[k], end[k])
	    ...
	    #print "In attach....working on thread_id %d" % thread_id
            thread_handle  = self.open_thread(thread_id)
	    #print "In attach....working on thread_handle %d" % thread_handle
            thread_context = self.get_thread_context(thread_handle)
	    #print "In attach....working on thread_context-eip %x" % thread_context.Eip
    ...
    def debug_event_iteration (self):
	    ...
	    # Get things into unsigned, don't know how else to do it for mac :(  SO LAME I AM
	    self.exception_address = (self.exception_address & 0xfffffff0) + (self.exception_address & 0x0000000f)  

	    #print "Got code in debug_event_iteration from pydbg_core: %d" % dbg.dwDebugEventCode
    ...
    def resume_thread (self, thread_id):
	    ...
	    #print "Oh shit, couldn't resume thread"
	...
	#print "Going to write to %x with %d bytes of data\n" % (address, length)
