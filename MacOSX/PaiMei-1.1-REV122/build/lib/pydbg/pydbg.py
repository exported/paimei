...
class pydbg(pydbg_core):
	...
	print "SETTING HARDWARE BREAKPOINT AT %x" % address
    ...
    def get_system_dlls(self):
	'''
	Return all the dll's that are loaded
	'''
	
	return pydbg_core.system_dlls
	...
	#print "*****************************Inside default breakpoint handler\n"                    
	self.first_breakpoint = False
	    ...
	    #print "Preparing to call user registered breakpoint handler"
	...
	print "Going to enumerate threads in snapshot"
        for thread_id in self.enumerate_threads():
	    print "Got a thread in snapshot with id %x" % thread_id
	    ...
	    print "Saving thread context of %x" % thread_id
	...
	print "Got baseaddr %x" % mbi.BaseAddress
	print "Got peb %x" % self.peb
        if mbi.BaseAddress == self.peb or mbi.BaseAddress in self.tebs.values():
            return "N/A"

#	print "Going to read from process memory at address %x" % address
        try:
            explored = self.read_process_memory(address, self.STRING_EXPLORATON_BUF_SIZE)
        except:
            return "N/A"
	
#	print "Done reading from process memory at address %x" % address
...
##########################################################################################
    def search_memory (pydbg, searchstring):
       '''
	Search memory
	@raise pdx: An exception is raised on failure.
	@rtype:     pydbg
	...
	'''
       ...
       do_not_snapshot = [PAGE_GUARD, PAGE_NOACCESS, PAGE_READONLY]
       cursor          = 0
	   ...
	   #print "Cursor is %x" % cursor
           ...
           try:
               mbi = pydbg.virtual_query(cursor)
           except:
           ...
           for has_protection in do_not_snapshot:
               if mbi.Protect & has_protection:
                   save_block = False
		   #print "Not snapshoting this one"
                   break
           ...
           if save_block:
               ...
               loc = 0
               m = re.search(searchstring, data[loc:])
               while(m):
                   loc = loc + m.span()[0] + 1
		   ...
		   rounded_size = ((32 + len(searchstring) - (16 - (addy - rounded_addy)) ) / 16) * 16  
