...
class module (pgraph.graph):
    ...
    def __init__ (self, name="", signature=None, depth=DEPTH_FULL, analysis=ANALYSIS_NONE):
        ...
        self.name      = name
	print "Setting self.base to MinEA!!!!!"
    	self.base      = MinEA() - 0x1000
        self.depth     = depth
	...
	# For Mac:
	# Initially self.base is the thing from the IDA file.  new_base is where it is loaded in memory for real
	# For the main app, self.base = 0, new_base = 1000.  We want to add nothing, so we really want self.base+=1000
	# For dlls, self.base = -1000, new_base = 91234500 or whatever.  We want to add 91234500, so we really want self.base+=1000.

	print "Rebasing with self=%x and new=%x\n" % (self.base, new_base)
