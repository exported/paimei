...
class process_stalker:
	...
	#print "In pstalker breakpoint handler!"
        if dbg.get_attr("first_breakpoint"):
            return DBG_CONTINUE

	#print "Not first breakpoint, logging"
        if self.print_bps:
	...
	#print "Looking for ", module, "in", self.pida_modules.keys()

	# for a mac, we look through all the modules and dll's we have and set any breakpoints.  This will only get called once at startup.
        if last_dll == None:
            region_line_re = re.compile('TEXT.*([0-9a-f]{8})-([0-9a-f]{8})\s+\[.+\]\s+[r-][w-][x-]/[r-][w-][x-]\s+[^\s]+\s+([^\s]+)')
	    (dummy,fp) = os.popen4("vmmap -w %d"%(self.attach))
            count = 0
            for x in fp:
                y = region_line_re.search(x)
                if y:
                     z = y.groups()
                     dll = system_dll(z[2], int(z[0],16), int(z[1],16))
                     self.set_bps(string.lower(os.path.basename(z[2])), dll)
	    return              # we treat the main module like its a dll
		...
		#print "This module we are setting breakpoints in has base %x" % self.pida_modules[module].base
		    ...
		    #print "Going to see if its filtered ", module
                    if self.filtered.has_key(module):
			#print "Checking for filtering %x but minus the base is %x\n" % (f.ea_start, f.ea_start - self.pida_modules[module].base)
    ...
    def stalk (self):
        ...
        for (target_id, tag_id) in self.filter_list:
			...
			#print "Filtering on ", hit.module
                        self.filtered[hit.module] = []

		    #print "Filtering %x but really %x\n" % (hit.eip, hit.eip - hit.base)
